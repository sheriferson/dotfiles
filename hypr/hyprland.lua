-- note: this is still a very cobbled together cargo-culted config
-- while I learn the basics of hyprland and decide what is needed,
-- what is broken, and what is actively undesirable. treat as untrusted for now
-- you can even see a mix of 2-space and 4-space indentation! oh the humanity

local mainMod = "SUPER" -- Sets "Command" key as main modifier
...
hl.curve("easy", { type = "spring", mass = 1, stiffness = 1400, dampening = 65 })
...
-- Lid: switch off built-in screen if the lid is closed
hl.bind("switch:on:Lid Switch", hl.dsp.exec_cmd("hyprctl eval 'hl.monitor({ output = \"eDP-1\", disabled = true})'"), { locked = true })
hl.bind("switch:off:Lid Switch", hl.dsp.exec_cmd("hyprctl eval 'hl.monitor({ output = \"eDP-1\", mode = \"preferred\", position = \"auto\", scale = 2.0, transform = 3, })'"), {locked = true})
...
hl.on("hyprland.start", function ()
  -- script to auto rotate the touchscreen based on orientation
  hl.exec_cmd("bash ~/.config/hypr/scripts/auto-rotate.sh")
end)
